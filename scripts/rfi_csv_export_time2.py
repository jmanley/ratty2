...
import ratty1, time, corr, numpy, struct, sys, logging, pylab, h5py, os, iniparse
...
def drawDataCallback(n_samples,indep,trig_level):
    ...
    pylab.hold(True)
    subplots[next_subplot].plot(freqs/1e6,calSpectrum,label='Signal on')
    pylab.hold(True)
    subplots[next_subplot].plot(freqs/1e6,emptySpectrum,label='Quiescent')
    subplots[next_subplot].legend()
    subplots[next_subplot].set_title('Spectrum of capture (%i samples)'%(len(unpackedData)))
...
def filewrite(adc_data_raw,spectrum_excl,spectrum_incl,timestamp,status):
    global cnt
    cnt=f['raw_dumps'].shape[0]-1
    print '  Adding entry %i...'%cnt,
    sys.stdout.flush()
    row=[timestamp,status['adc_overrange'],status['fft_overrange'],status['adc_shutdown'],status['adc_level'],status['input_level']
    row.extend(adc_data_raw)
    fc.writerow(row)
    print 'done'
